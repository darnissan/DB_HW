...
Views = ["Owner_reviews", "Owner_city_apartments", "Customer_apartments", "Cust1_apartments"
    , "Apartment_ratios", "Apartment_approximations","AvgNightlyPrice","AvgRating"]
...
def create_tables():
    ...
    attributes = []
    attributes.append(
        "customer_id INTEGER  REFERENCES Customer(customer_id) ON DELETE SET NULL CHECK (customer_id IS NULL OR customer_id > 0)")
    attributes.append(
        "apartment_id INTEGER  REFERENCES Apartment(apartment_id) ON DELETE SET NULL CHECK (apartment_id > 0 OR apartment_id IS NULL)")
...
def make_avg_nightly_price ()  :

    attributes = []
    attributes.append(" SELECT r.Apartment_ID, ")
    attributes.append(" AVG(r.total_price / NULLIF(r.end_date - r.start_date, 0)) AS avg_nightly_price")
    attributes.append("FROM reservations r" )
    attributes.append("GROUP BY r.Apartment_ID")
    make_view("AvgNightlyPrice", attributes)

def make_avg_rating() :
    attributes = []
    attributes.append("SELECT rev.apartment_id,")
    attributes.append("AVG(rev.rating) AS reviews_avg_rating")
    attributes.append("FROM Apartment_Reviews rev")
    attributes.append("GROUP BY rev.apartment_id")
    make_view("AvgRating",attributes)
...
def make_customer_ratio(customer_ID):
    ...
    attributes.append("WHERE Apartment_Reviews.customer_id="+str(customer_ID))
...
def get_apartment(apartment_id: int) -> Apartment:
    ...
    try:
        ...
        if apts is not None and rows_effected > 0:
            result = Apartment(apts[0]['apartment_id'], apts[0]['address'], apts[0]['city'], apts[0]['country'],
                               apts[0]['size'])
        else:
...
def get_apartment_owner(apartment_id: int) -> Owner:
    ...
    try:
        conn = Connector.DBConnector()
        query = sql.SQL(
            "SELECT * FROM Owner WHERE owner_id IN (SELECT owner_id FROM Owner_Apartments WHERE apartment_id={0})".format(
                apartment_id
            )
        )
...
def get_owner_apartments(owner_id: int) -> List[Apartment]:
    ...
    try:
        conn = Connector.DBConnector()
        query = sql.SQL(
            "SELECT * FROM Apartment WHERE apartment_id IN (SELECT apartment_id FROM Owner_Apartments WHERE owner_id={0})".format(
                owner_id))
        ...
        if (apartments is not None) and rows > 0:
            for index in range(apartments.size()):
                ...
                result.append(Apartment(current_row['Apartment_ID'], current_row['Address'], current_row['City'],
                                        current_row['Country'], current_row['Size']))
    except Exception as e:
        print(e)
        return []
    finally:
...
def get_owner_rating(owner_id: int) -> float:
    ...
    try:
        make_owner_reviews()
        conn = Connector.DBConnector()
        sub_query = "(SELECT AVG(COALESCE(Rating,0)) AS avg_rating FROM Owner_Reviews WHERE Owner_ID=" + str(
            owner_id) + " GROUP BY Apartment_ID) AS subquery;"
...
def best_value_for_money() -> Apartment:
    ...
    result = None
    try:
        make_avg_nightly_price()
        make_avg_rating()
        query = """
             SELECT Apartment_ID, Address, City, Country, Size, value_for_money
                FROM (
                    SELECT a.Apartment_ID, a.Address, a.City, a.Country, a.Size,
                           (ar.reviews_avg_rating / ap.avg_nightly_price) AS value_for_money
                    FROM Apartment a
                    JOIN AvgNightlyPrice ap ON a.Apartment_ID = ap.Apartment_ID
                    JOIN AvgRating ar ON a.Apartment_ID = ar.apartment_id
